...
let firebaseCertificatesExpiresAt = 0;
...
async function getFirebaseCertificates() {
  if (firebaseCertificates && Date.now() < firebaseCertificatesExpiresAt) return firebaseCertificates;
  ...
  firebaseCertificatesExpiresAt = Date.now() + seconds * 1000;
...
async function verifyFirebaseToken(request, projectId) {
  ...
  if (!certificate) throw new Error("認証トークンの署名鍵が見つかりません。");
  const key = await crypto.subtle.importKey("spki", pemToDer(certificate), { name: "RSASSA-PKCS1-v1_5", hash: "SHA-256" }, false, ["verify"]);
