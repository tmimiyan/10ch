...
import { addDoc, collection, getDocs, limit, onSnapshot, orderBy, query, serverTimestamp, where } from "https://www.gstatic.com/firebasejs/11.10.0/firebase-firestore.js";
...
let threadNotificationsReady = false;
initNotifications();
...
loadThreads();
// Do not notify for the initial list; only notify for later Firestore additions.
onSnapshot(query(collection(db, "threads"), orderBy("createdAt", "desc"), limit(30)), (snapshot) => {
  if (threadNotificationsReady) snapshot.docChanges().filter((change) => change.type === "added").forEach((change) => {
    const thread = change.doc.data();
    if (thread.authorId !== currentUser?.uid) showOpenPageNotification("新しいスレッド", thread.title || "10ちゃんねる", `./thread.html?id=${encodeURIComponent(change.doc.id)}`);
  });
  threadNotificationsReady = true;
}, console.warn);
