...
// A submit event can be fired twice by a rapid tap/click. Keep one shared lock
// for this page so only one Firestore write is in progress at a time.
...
// Protect against a duplicated script tag left by an accidental partial deploy.
// The marker is on the form, so it works even when two module versions load.
if (!replyForm.dataset.submitHandlerAttached) {
  replyForm.dataset.submitHandlerAttached = "true";
  replyForm.addEventListener("submit", async (event) => { event.preventDefault(); if (isReplySubmitting) return; if (!currentUser) { showToast("投稿にはログインが必要です。"); location.href = "./login.html"; return; } const body = $("#reply-body").value.trim(); const inputAuthorName = $("#reply-author").value.trim(); if (!isAdminUser(currentUser) && inputAuthorName.includes("👑")) { showToast("王冠の絵文字は管理者専用です。"); return; } const author = postAuthor(currentUser, inputAuthorName, $("#reply-author-color").value); const authorProfileVisible = isAdminUser(currentUser) || Boolean(inputAuthorName && inputAuthorName !== "名無しさん"); const imageFiles = [...$("#reply-image").files]; if ((!body && !imageFiles.length) || !id) { showToast("本文または画像を入力してください。"); return; } const imageError = validateImages(imageFiles); if (imageError) { showToast(imageError); return; } isReplySubmitting = true; replyButton.disabled = true; let images = []; try { const firstLoginAt = await getFirstLoginAt(currentUser); const threadRef = doc(db, "threads", id); images = await uploadPostImages(imageFiles, currentUser.uid); const replyRef = doc(collection(threadRef, "replies")); const batch = writeBatch(db); batch.set(replyRef, { body, authorId: currentUser.uid, authorName: author.name, authorColor: author.color, authorProfileVisible, authorFirstLoginAt: firstLoginAt, imageUrls: images.map((image) => image.url), imagePaths: images.map((image) => image.path), imageUrl: images[0]?.url || null, createdAt: serverTimestamp(), }); batch.update(threadRef, { replyCount: increment(1) }); await batch.commit(); requestPostNotification({ type: "reply", threadId: id, replyId: replyRef.id }); replyForm.reset(); lockAuthorInputs(currentUser); } catch (error) { await removePostImages(images).catch(console.warn); showToast(firebaseMessage(error)); } finally { isReplySubmitting = false; replyButton.disabled = false; } });
}
