...
replyForm.addEventListener("submit", async (event) => { event.preventDefault(); if (!currentUser) { showToast("投稿にはログインが必要です。"); location.href = "./login.html"; return; } const body = $("#reply-body").value.trim(); const authorName = $("#reply-author").value.trim() || "名無しさん"; if (!body || !id) return; replyButton.disabled = true; try { const threadRef = doc(db, "threads", id); await addDoc(collection(threadRef, "replies"), { body, authorId: currentUser.uid, authorName, createdAt: serverTimestamp() }); await updateDoc(threadRef, { replyCount: increment(1) }); replyForm.reset(); await loadThread(); } catch (error) { showToast(firebaseMessage(error)); } finally { replyButton.disabled = false; } });
loadThread();
