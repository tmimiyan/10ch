...
import { initNotifications, showOpenPageNotification } from "./notifications.js";
...
let repliesNotificationsReady = false;
initNotifications();
...
async function loadThread() {
  ...
  try {
    ...
    const thread = snapshot.data(); currentThread = thread; document.title = `${thread.title} | 10ちゃんねる`;
    ...
    unsubscribeReplies = onSnapshot(repliesQuery, (snapshot) => {
      if (repliesNotificationsReady) snapshot.docChanges().filter((change) => change.type === "added").forEach((change) => {
        const reply = change.doc.data();
        if (reply.authorId !== currentUser?.uid) showOpenPageNotification(`新しいレス: ${thread.title}`, (reply.body || "画像が投稿されました。").slice(0, 120), `./thread.html?id=${encodeURIComponent(id)}`);
      });
      repliesNotificationsReady = true;
