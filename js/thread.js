...
function updateThreadDeleteButton() { clearTimeout(threadDeleteTimer); const ownEmptyThread = canDeletePost(currentThread) && currentThread?.replyCount === 0; deleteThreadButton.hidden = !(isAdmin(currentUser) || ownEmptyThread); if (!isAdmin(currentUser) && ownEmptyThread) { const remaining = currentThread.createdAt.toMillis() + 5 * 60 * 1000 - Date.now(); threadDeleteTimer = setTimeout(updateThreadDeleteButton, Math.max(0, remaining) + 50); } }
...
function syncAuth(user) { currentUser = user; if (!user) { location.replace("./login.html"); return; } lockAuthorInputs(user); getFirstLoginAt(user).catch(console.warn); $(".login-link").hidden = Boolean(user); $(".logout-button").hidden = !user; replyButton.disabled = !user; updateThreadDeleteButton(); $("#login-notice").textContent = `${displayName(user)} として投稿します。`; if (latestReplies) renderReplies(latestReplies); loadThread(); }
...
function authorNode(name, color) { const author = document.createElement("span"); author.textContent = name || "名無しさん"; if (/^#[0-9a-f]{6}$/i.test(color || "")) author.style.color = color; if (isAdminPost(name, color)) author.append(document.createTextNode(" 👑")); return author; }
...
function replyNode(reply, number, replyId) { const article = document.createElement("article"); article.className = "reply"; const meta = document.createElement("div"); meta.className = "reply-meta"; const author = authorNode(reply.authorName, reply.authorColor); meta.append(`#${number}  `, author); if (isFirstThreeDays(reply.authorFirstLoginAt)) meta.append(" 🍀"); meta.append(`　${formatDate(reply.createdAt)}`); article.append(meta); if (reply.body) { const body = document.createElement("p"); body.className = "post-body"; appendLinkedText(body, reply.body); article.append(body); } const images = postImages(reply.imageUrls, reply.imageUrl); if (images) article.append(images); if (canDeletePost(reply)) { const button = document.createElement("button"); button.className = "button button-danger reply-delete delete-icon-button"; button.type = "button"; button.setAttribute("aria-label", "レスを削除"); button.title = "レスを削除"; const icon = document.createElement("i"); icon.className = "fas fa-trash"; icon.setAttribute("aria-hidden", "true"); button.append(icon); button.addEventListener("click", () => deleteReply(replyId, reply)); article.append(button); if (!isAdmin(currentUser)) { const remaining = reply.createdAt.toMillis() + 5 * 60 * 1000 - Date.now(); setTimeout(() => button.remove(), Math.max(0, remaining) + 50); } } return article; }
