...
function syncAuth(user) { currentUser = user; $(".login-link").hidden = Boolean(user); $(".logout-button").hidden = !user; replyButton.disabled = !user; deleteThreadButton.hidden = !isAdmin(user); $("#login-notice").textContent = user ? `${displayName(user)} として投稿します。` : "投稿にはログインが必要です。"; if (latestReplies) renderReplies(latestReplies); }
