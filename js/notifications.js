...
async function syncNotificationDevice(user = auth.currentUser) {
  const subscriptionId = oneSignal?.User?.PushSubscription?.id;
  if (user && subscriptionId && oneSignal?.User?.PushSubscription?.optedIn) {
...
export function initNotifications() {
  ...
  window.OneSignalDeferred.push(async (sdk) => {
    try {
      // GitHub Pages のリポジトリ配下でも正しいWorker URLとスコープを自動計算します。
      ...
      syncNotificationDevice().catch(console.warn);
      onAuthStateChanged(auth, (user) => syncNotificationDevice(user).catch(console.warn));
    } catch (error) {
      console.warn("OneSignal initialization failed", error);
      const detail = error instanceof Error ? error.message : String(error || "");
      showToast(`通知サービスを初期化できませんでした。${detail ? ` ${detail}` : ""}`);
    }
  });
}
